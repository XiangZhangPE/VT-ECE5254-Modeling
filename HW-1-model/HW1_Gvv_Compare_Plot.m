...
sim_data = readmatrix('BuckBoost_Gvv.xlsx','Sheet','Sheet1');
...
for j=1:length(wout)
     ...
     Gvv(j) = -(D*Rl*(C*Rc*s + 1))/((D - 1)*(Rl + Le*s + C*Le*Rc*s^2 + C*Le*Rl*s^2 + C*Rc*Rl*s));

     % Bode Magnitude Calculation
     magnitude_model(j) = 20*log10(abs(Gvv(j)));
     
     % Phase Calculation
     phase_model(j) = 180*angle(Gvv(j))/pi+180;
end
