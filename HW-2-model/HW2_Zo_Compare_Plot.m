...
% Gvv = -(D*Rl*Vin*(C*Rc*s + 1))/((D - 1)*(Rl + Le*s + C*Le*Rc*s^2 + C*Le*Rl*s^2 + C*Rc*Rl*s));
...
sim_data = readmatrix('BuckBoost_Zo.xlsx','Sheet','Sheet1');
...
for j=1:length(wout)
     ...
     Zo(j) = (Le*Rl*s*(C*Rc*s + 1))/(Rl + Le*s + C*Le*Rc*s^2 + C*Le*Rl*s^2 + C*Rc*Rl*s);
